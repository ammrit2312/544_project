...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask: Optional[torch.Tensor]=None):
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, d_model: int, num_heads: int, num_layers: int, d_ff: int, max_len: int=5000, dropout: float=0.1, pad_idx: int=0):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, tokens: torch.Tensor, encoder_output: torch.Tensor, encoder_padding_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        tgt_padding_mask = self.generate_padding_mask(tokens)
