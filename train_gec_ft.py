...
def main():
    ...
    model = GECModel(d_model=D_MODEL, num_heads=NUM_HEADS, d_ff=D_FF, num_layers=NUM_LAYERS, max_len=MAX_LEN, freeze_encoder=False).to(device)
    encoder_params = [p for p in model.encoder.parameters() if p.requires_grad]
    head_params = [p for p in list(model.projection.parameters()) + list(model.decoder.parameters()) if p.requires_grad]
    trainable = encoder_params + head_params
    print(f'Trainable params: {sum((p.numel() for p in trainable)):,}')
    optimizer = torch.optim.AdamW([{'params': encoder_params, 'lr': ENCODER_LR}, {'params': head_params, 'lr': DECODER_LR}])
    total_steps = len(train_loader) * EPOCHS
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=total_steps // 10, num_training_steps=total_steps)
    criterion = nn.CrossEntropyLoss(ignore_index=-100)
    os.makedirs('checkpoints', exist_ok=True)
    best_val_loss = float('inf')
    cfg = {'freeze_encoder': False, 'd_model': D_MODEL, 'num_heads': NUM_HEADS, 'd_ff': D_FF, 'num_layers': NUM_LAYERS, 'max_len': MAX_LEN, 'encoder_lr': ENCODER_LR, 'decoder_lr': DECODER_LR}
    for epoch in range(EPOCHS):
        ...
        if avg_val < best_val_loss:
            ...
            torch.save({'epoch': epoch + 1, 'model_state': model.state_dict(), 'val_loss': avg_val, 'config': cfg}, CHECKPOINT)
