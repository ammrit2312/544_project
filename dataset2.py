...
class GECDataset(Dataset):
    ...
    def __init__(self, csv_path: str, tokenizer: PreTrainedTokenizerBase, max_source_len: int=128, max_target_len: int=128, source_col: str='incorrect', target_col: str='correct', lang_col: str='lang', add_lang_token: bool=True):
        ...
        if self.bos_token_id is None or self.eos_token_id is None or self.pad_token_id is None:
            raise ValueError('Tokenizer must define cls_token_id, sep_token_id, and pad_token_id. XLM-R tokenizer is a good fit here.')
    ...
    def _encode_source(self, text: str) -> Dict[str, torch.Tensor]:
        encoded = self.tokenizer(text, max_length=self.max_source_len, truncation=True, padding='max_length', return_tensors='pt', add_special_tokens=True)
    ...
    def _encode_target(self, text: str) -> torch.Tensor:
        encoded = self.tokenizer(text, max_length=self.max_target_len, truncation=True, padding='max_length', return_tensors='pt', add_special_tokens=True)['input_ids'].squeeze(0)
        return encoded
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        target_input_ids = torch.cat([target_input_ids, torch.full((1,), self.pad_token_id, dtype=torch.long)])
        target_labels = torch.cat([target_labels, torch.full((1,), self.pad_token_id, dtype=torch.long)])
        return {'source_ids': source['input_ids'], 'source_mask': source['attention_mask'], 'target_input_ids': target_input_ids, 'target_labels': target_labels, 'lang': lang}
...
class GECDataCollator:
    ...
    def __call__(self, batch):
        ...
        return {'source_ids': source_ids, 'source_mask': source_mask, 'target_input_ids': target_input_ids, 'target_labels': target_labels, 'langs': [item.get('lang') for item in batch]}
if __name__ == '__main__':
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained('xlm-roberta-base')
    ds = GECDataset(csv_path='data/processed/train.csv', tokenizer=tokenizer, max_source_len=32, max_target_len=32)
    sample = ds[0]
