...
class MT5GECDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lab = self.tok(text_target=tgt, max_length=self.max_tgt, truncation=True, padding='max_length', return_tensors='pt')
        labels = lab['input_ids'].squeeze().clone()
        labels[labels == self.pad_id] = -100
        return {'input_ids': enc['input_ids'].squeeze(), 'attention_mask': enc['attention_mask'].squeeze(), 'labels': labels}

def collate(batch):
    return {'input_ids': torch.stack([b['input_ids'] for b in batch]), 'attention_mask': torch.stack([b['attention_mask'] for b in batch]), 'labels': torch.stack([b['labels'] for b in batch])}
...
def main():
    ...
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=nw, pin_memory=device.type == 'cuda', collate_fn=collate)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=nw, pin_memory=device.type == 'cuda', collate_fn=collate)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f'Epoch {epoch}/{EPOCHS} | Train loss: {tr:.4f} | Val loss: {va:.4f} | {(time.time() - t0) / 60:.1f}m')
        if va < best_val:
            best_val = va
            torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'val_loss': va, 'model_name': MODEL_NAME}, CHECKPOINT)
