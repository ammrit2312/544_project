...
LR             = 2e-5
MAX_LEN        = 128
TOP_K_LABELS   = 5000   # keep only top-K edit labels by frequency
...
def build_label_vocab(csv_path: str, top_k: int, lang: str | None = None) -> list[str]:
    ...
    fixed = [KEEP, DELETE]
    top = [lbl for lbl, _ in counter.most_common(top_k)
           if lbl not in fixed]
    return fixed + top
...
def align_labels(src_tokens: list[str], tgt_tokens: list[str]) -> list[str]:
    ...
    alignment = []  # list of (src_idx, tgt_idx) for matched tokens
    i, j = m, n
    while i > 0 and j > 0:
        if src_tokens[i-1] == tgt_tokens[j-1]:
    ...
    insert_map: dict[int, list[str]] = {}  # src_idx → list of tokens to insert before it
    prev_src = -1
    tgt_ptr = 0
    for src_i, tgt_i in alignment:
        while tgt_ptr < tgt_i:
            if tgt_ptr not in matched_tgt:
                insert_map.setdefault(src_i, []).append(
                    f'$INSERT_{tgt_tokens[tgt_ptr]}'
                )
            tgt_ptr += 1
        tgt_ptr = tgt_i + 1
    ...
    for src_i, src_tok in enumerate(src_tokens):
        ...
        if src_i in matched_src:
            ...
            tgt_i = next(
                (t for s, t in alignment if s > src_i), None
            )
...
class GECTorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids      = enc['input_ids'].squeeze()       # [max_len]
        attention_mask = enc['attention_mask'].squeeze()  # [max_len]
        ...
        label_ids = [
            self.label2id.get(lbl, self.keep_id)
...
class GECTorModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        hidden = self.dropout(out.last_hidden_state)   # [batch, seq, hidden]
        return self.classifier(hidden)                 # [batch, seq, num_labels]
...
def run_epoch(model, loader, optimizer, criterion, device, scheduler=None, train=True):
    ...
    with context:
        for batch in loader:
            ...
            logits = model(input_ids, attention_mask)   # [B, seq, num_labels]
