...
MAX_LEN = 128
_DEFAULT_CFG = {'freeze_encoder': False, 'd_model': 512, 'num_heads': 8, 'd_ff': 2048, 'num_layers': 4, 'max_len': 128}
...
model = GECModel(d_model=merged['d_model'], num_heads=merged['num_heads'], d_ff=merged['d_ff'], num_layers=merged['num_layers'], max_len=max_len, freeze_encoder=merged['freeze_encoder']).to(device)
...
def correct(sentence: str, max_new_tokens: int | None=None) -> str:
    ...
    src = tokenizer(sentence, max_length=max_len, padding='max_length', truncation=True, return_tensors='pt')
